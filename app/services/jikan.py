...
class JikanService(BaseAPIService):
    ...
    async def search(
        ...
        logger.info(f"Searching Jikan {media_type} for: {query} (limit: {limit})")
    ...
    async def get_by_id(
        ...
        logger.info(f"Fetching Jikan {media_type} by ID: {media_id}")
        data = await self._get(f"{media_type}/{media_id}/full")
        if data and data.get("data"):
            logger.debug(f"Found {media_type}: {data['data'].get('title', 'Unknown')}")
