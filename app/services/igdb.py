...
logger = setup_logger("igdb_service")
...
class IGDBService(BaseAPIService):
    ...
    async def _check_auth(self) -> bool:
        ...
        logger.info("Generating new IGDB access token...")
    ...
    def _save_token_to_env(self, token: str, expires_at: Optional[datetime] = None):
        ...
        try:
            ...
            logger.info(f"Saved IGDB access token to {env_path}")
    ...
    async def search(self, query: str, limit: int = 10) -> List[dict]:
        """Search games."""
        logger.info(f"Searching IGDB for: {query} (limit: {limit})")
    ...
    async def get_by_id(self, media_id: str) -> Optional[dict]:
        """Get game details."""
        logger.info(f"Fetching IGDB game by ID: {media_id}")
        ...
        if data and len(data) > 0:
            logger.debug(f"Found game: {data[0].get('name', 'Unknown')}")
            return data[0]
