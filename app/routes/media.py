...
from app.models import MediaTypeEnum, User
from app.routes.deps import get_current_user
from app.schemas import (AnimeCreate, AnimeResponse, AnimeUpdate, BookCreate,
...
async def upload_image(
    ...
    file_path = Path(__file__).parent.parent.parent / f"static/images/{unique_filename}"
...
def create_movie(
    ...
    return media_crud.create_movie(db, obj_in=movie)
...
def update_movie(
    ...
    updated_movie = media_crud.update_movie(db, id=movie_id, obj_in=movie)
...
def delete_movie(
    ...
    if not media_crud.delete(db, id=movie_id):
        raise HTTPException(status_code=404, detail="Movie not found")
...
def create_series(
    ...
    return media_crud.create_series(db, obj_in=series)
...
def update_series(
    ...
    updated_series = media_crud.update_series(db, id=series_id, obj_in=series)
...
def delete_series(
    ...
    if not media_crud.delete(db, id=series_id):
        raise HTTPException(status_code=404, detail="Series not found")
...
def create_anime(
    ...
    return media_crud.create_anime(db, obj_in=anime)
...
def update_anime(
    ...
    updated_anime = media_crud.update_anime(db, id=anime_id, obj_in=anime)
...
def delete_anime(
    ...
    if not media_crud.delete(db, id=anime_id):
        raise HTTPException(status_code=404, detail="Anime not found")
...
def create_manga(
    ...
    return media_crud.create_manga(db, obj_in=manga)
...
def update_manga(
    ...
    updated_manga = media_crud.update_manga(db, id=manga_id, obj_in=manga)
...
def delete_manga(
    ...
    if not media_crud.delete(db, id=manga_id):
        raise HTTPException(status_code=404, detail="Manga not found")
...
def create_book(
    ...
    return media_crud.create_book(db, obj_in=book)
...
def update_book(
    ...
    updated_book = media_crud.update_book(db, id=book_id, obj_in=book)
    if not updated_book:
        raise HTTPException(status_code=404, detail="Book not found")
    return updated_book
...
def delete_book(
    ...
    if not media_crud.delete(db, id=book_id):
        raise HTTPException(status_code=404, detail="Book not found")
...
def create_game(
    ...
    return media_crud.create_game(db, obj_in=game)
...
def update_game(
    ...
    updated_game = media_crud.update_game(db, id=game_id, obj_in=game)
    if not updated_game:
        raise HTTPException(status_code=404, detail="Game not found")
    return updated_game
...
def delete_game(
    ...
    if not media_crud.delete(db, id=game_id):
        raise HTTPException(status_code=404, detail="Game not found")
